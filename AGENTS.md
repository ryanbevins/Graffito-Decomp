...
- **Prove stack/register-only claims with the noise checker**. Before writing that a function has no functional gap left and is only stack/register/regswap noise, run `python tools/check-diff-noise.py -u <objdiff-unit> -d '<function substring>' --show-ignored` after a successful `python configure.py --non-matching && ninja`. Treat any nonzero exit or any `structural:*` category as a real structural/functional gap. Even when the command exits 0, only `ignored:register`, `ignored:stack`, and `ignored:register+stack` are operator-forgiven as stack/regswap residue; branch-destination-only or commutative-source-swap rows must be investigated or documented separately and must not be called stack/regswap noise.
