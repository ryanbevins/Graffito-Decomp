...
void TMarioEffect::perform(u32 flags, JDrama::TGraphics* gfx)
{
	...
	if ((flags & 2) && unk7C != 0) {
		if ((*(TWaterGun**)((u8*)unk68 + 0x3E4))->getEmitMtx(0) != nullptr) {
			PSMTXCopy((*(TWaterGun**)((u8*)unk68 + 0x3E4))->getEmitMtx(0),
			          unk80->getModel()->unk20);
			unk80->perform(2, gfx);
		}
	}
...
void TMarioEffect::setJumpIntoWaterEffect()
{
	...
	PSMTXCopy(mtx, (*actor)->getModel()->unk20);

	(*actor)->getModel()->mShapePackets[0].show();
	(*actor)->getModel()->mShapePackets[1].show();
	(*actor)->getModel()->mShapePackets[2].show();
	(*actor)->getModel()->mShapePackets[3].show();
	(*actor)->getModel()->mShapePackets[4].show();
	unk6C[idx] = 1;
}
