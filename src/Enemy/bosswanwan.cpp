...
void TBWBinder::bind(TLiveActor* actor)
{
	...
	if (&TNerveBWJumpToBath::theNerve()
	        == actor->mSpine->getLatestNerve()
	    || &TNerveBWDie::theNerve() == actor->mSpine->getLatestNerve()) {
		actor->mLinearVelocity = nextPos - actor->mPosition;
		return;
	}
...
DEFINE_NERVE(TNerveBWShake, TLiveActor)
{
	...
	if (actor->curAnmEndsNext(0, nullptr)) {
		...
		self->unk194 = 0;
		self->unk17C = 0;
		const JGeometry::TVec3<f32>* soundPos = &self->mPicket->mPosition;
		if (gpMSound->gateCheck(0x2967))
			MSoundSESystem::MSoundSE::startSoundActor(
			    0x2967, soundPos, 0, nullptr, 0, 4);
		return true;
	}
