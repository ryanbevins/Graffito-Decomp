...
{
	static const int spawnCounts[] = { 1, 2, 3, 4, 4 };

	const int count = spawnCounts[generation];
	f32 start       = 2.0f * 3.1415927f * (rand() * 0.000030517578f);
	...
	for (int i = 0; i < count; ++i) {
		...
		f32 angle = start + (2.0f * 3.1415927f * i) / count;
		manta->mDirection.set(sinf(angle), 0.0f, cosf(angle));
...
void TBossMantaManager::createEnemies(int count)
{
	...
	if (unk38 && mObjNum + count > unk38->mSLActiveEnemyNum.get())
		count = unk38->mSLActiveEnemyNum.get() - mObjNum;
...
{
	f32 width  = SMSGetGameRenderWidth();
	f32 height = SMSGetGameRenderHeight();
	...
	GXPosition3f32(0.0f, height, -10.0f);
	GXPosition3f32(width, height, -10.0f);
	GXPosition3f32(width, 0.0f, -10.0f);
...
void TBossMantaManager::drawMantaShadow(JDrama::TGraphics* graphics)
{
	...
	int active = getActiveObjNum();
	for (int i = 0; i < active; ++i) {
		TBossManta* manta = (TBossManta*)getObj(i);
		if (!manta || (manta->mLiveFlag & (LIVE_FLAG_DEAD | LIVE_FLAG_CLIPPED_OUT)))
			continue;

		J3DModel* model = manta->getModel();
		if (!model || !model->getModelData())
			continue;

		u16 shapeNum = model->getModelData()->getShapeNum();
		GXSetCullMode(GX_CULL_FRONT);
		GXSetBlendMode(GX_BM_BLEND, GX_BL_ONE, GX_BL_ONE, GX_LO_NOOP);
		for (u16 j = 0; j < shapeNum; ++j)
			model->getShapePacket(j)->draw();
	...
	u16 width  = (u16)SMSGetGameRenderWidth();
	u16 height = (u16)SMSGetGameRenderHeight();
	GXSetTexCopySrc(0, 0, width, height);
	GXSetTexCopyDst(width, height, GX_TF_I8, GX_FALSE);
	...
	GXTexObj texObj;
	GXInitTexObj(&texObj, mEfbAlpha, width, height, GX_TF_I8, GX_CLAMP,
	             GX_CLAMP, GX_FALSE);
	...
	if (mBattleState.mState == 2) {
		++mShadowAlphaTimer;
		if (mShadowAlphaTimer > 15)
			mShadowAlphaTimer = 15;
	}
	...
	GXBegin(GX_QUADS, GX_VTXFMT0, 4);
	GXPosition3f32(0.0f, screenHeight, -10.0f);
	GXTexCoord2f32(0.0f, 0.0f);
	GXPosition3f32(screenWidth, screenHeight, -10.0f);
	GXTexCoord2f32(1.0f, 0.0f);
	GXPosition3f32(screenWidth, 0.0f, -10.0f);
                                            ...
                                            JDrama::TGraphics* graphics)
{
	...
	for (int i = 0; i < mColCount; ++i) {
		...
		if (other->getActorType() != (ACTOR_TYPE_PLAYER | 1))
			continue;
...
void TBossMantaManager::TMantaMessageState::update()
{
	...
	case 1: {
		int count  = 0;
		int active = mManager->getActiveObjNum();
		for (int i = 0; i < active; ++i) {
...
BOOL TBossManta::getIntoGraphVec(JGeometry::TVec3<f32>* out)
{
	...
	for (int i = 0; i < 12; ++i) {
		...
		if (dx * edgeZ - dz * edgeX < 0.0f) {
			out->set(edgeZ, 0.0f, -edgeX);
			out->normalize();
			return true;
		}
	}
