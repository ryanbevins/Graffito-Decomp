...
void TBossTelesa::init(TLiveManager* manager)
{
	...
	SMS_ChangeTextureAll(mMActor->unk4->mModelData, "H_ma_rak_dummy",
	                     *texture);
...
void TBossTelesa::genAttacker()
{
	...
	for (int i = 0; i < count; ++i) {
		...
		f32 posY = rootMtx[1][3];
		f32 posZ = rootMtx[2][3];
		f32 posX = rootMtx[0][3];
		posY -= 50.0f;
		bubble->mPosition.x = posX;
		bubble->mPosition.y = posY;
		bubble->mPosition.z = posZ;
...
void TBossTelesa::setSpicy(TLiveActor* actor)
{
	...
	if (oldAnmActor->unkC) {
		J3DAnmTransform* oldAnmArg = oldAnm;
		oldAnmActor->unkC->setOldMotionBlendAnmPtr(oldAnmArg);
	}
