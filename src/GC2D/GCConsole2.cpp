...
static inline int getOffsetForAboveScreen(TExPane* pane)
{
...
static inline void setEmitterToPaneCenter(JPABaseEmitter* emitter, J2DPane* pane)
{
	JUTRect bounds(pane->mGlobalBounds);
	emitter->unk160.x = bounds.x1 + bounds.getWidth() * 0.5f;
	emitter->unk160.y = bounds.y1 + bounds.getHeight() * 0.5f;
	emitter->unk160.z = 0.0f;
}
...
static inline void drawPictureWithTextureSize(J2DPicture* picture, int x, int y)
{
	...
	picture->draw(x, y, texture->mWidth, texture->mHeight, false, false,
	              false);
}
...
static inline bool isMountedYoshi(TMario* mario)
{
	...
	return yoshi != nullptr && yoshi->mState == TYoshi::MOUNTED;
}
...
static inline void playHudMoveSound(u32 soundID)
{
	...
	if (gpMarioOriginal->mHealth == 0)
		return;
	if ((s16)gpMarioOriginal->unk12C == 0)
		return;
...
static inline void processBalloonTextStep(TGCConsole2* console)
{
	...
	JSUMemoryOutputStream* output
	    = (JSUMemoryOutputStream*)console->unk3D8;
		...
		if ((CONSOLE)->unk1CC[0] >= 4) {                                       \
			if (AIR_MODE) {                                                    \
				setPictureColor((CONSOLE)->unk178->getPane(), 0x00FFFFFF,      \
				                0x003CFF00);                                  \
				setPictureColor((CONSOLE)->unk17C[0], 0x00FFFFFF,              \
				                0x003CFF00);                                  \
			} else {                                                           \
			...
			if (AIR_MODE) {                                                    \
				setPictureColor((CONSOLE)->unk178->getPane(), 0x0010FFFF,      \
				                0x003CFF00);                                  \
				setPictureColor((CONSOLE)->unk17C[0], 0x0010FFFF,              \
				                0x003CFF00);                                  \
			} else {                                                           \
...
static inline void playLifeChangeSound(u32 sound)
{
	if (gpMarDirector->mState == TMarDirector::STATE_UNK4
	    && gpMarDirector->unk124 == 0 && gpMSound->gateCheck(sound)) {
...
static inline void updateLifeMeterState(TGCConsole2* console)
{
	...
	case 0:
		...
		if (console->unk1CC[0] < 8 && !lifePane->isVisible()
		    && amount != 0) {
...
#define SET_BLEND_DIGIT(PANE, TEXTURES, DIGIT)                                 \
	do {                                                                       \
		(PANE)->setPaneBlend(10, (TEXTURES)[DIGIT], nullptr);                  \
		EMIT_COUNTER_PARTICLE(PANE);                                           \
	} while (0)
	...
	if (console->unk8A != 0) {
		if (console->unk8A > 0xFB) {
			...
			if (console->unk170 != target) {
				...
				if (console->unk170 < 0) {
					console->unk170 = 0;
				} else if (gpMSound->gateCheck(0x4850)) {
		...
		if (console->unk8A == 0xFC) {
			...
			if (value < 100) {
				...
				if (value % 10 == 0)
					SET_BLEND_DIGIT(console->unk134[0], console->unkE0,
					                value / 10);

				SET_BLEND_DIGIT(console->unk134[1], console->unkE0,
				                value % 10);
			} else {
				int hundreds = value / 100;
				if (value % 100 == 0) {
				...
				SET_BLEND_DIGIT(console->unk134[2], console->unkE0,
				                value % 10);
			}
...
static inline bool updateUpPane(TBoundPane* pane)
{
	if (pane->update()) {
		if (pane->unk14.x1 == 0 && pane->unk14.y1 == 0)
			return true;
...
static inline bool updateUpBlendPane(TBlendPane* pane)
{
	if (pane->update()) {
		if (pane->unk14.x1 == 0 && pane->unk14.y1 == 0)
			return true;
...
void TGCConsole2::load(JSUMemoryInputStream& stream)
{
	...
	for (int i = 0; i < 9; ++i) {
		unk17C[i * 2]     = unkB0->search('lm01' + i * 0x100);
		unk17C[i * 2 + 1] = unkB0->search('lm02' + i * 0x100);
		unk1D0[i]         = unk17C[i * 2]->mBounds;
	}
	...
	// Codegen note: raw reads the picture white colors from these timer panes.
...
void TGCConsole2::startCameraDemo()
{
	...
	unk274->setPanePosition(1, JUTPoint(0, 0), JUTPoint(0, 0),
	                         JUTPoint(0, 0));
	unk274->update();
	unk26C->setPanePosition(1, JUTPoint(0, 0), JUTPoint(0, 0),
	                         JUTPoint(0, 0));
	unk26C->update();
	unk270->setPanePosition(1, JUTPoint(0, 0), JUTPoint(0, 0),
	                         JUTPoint(0, 0));
	unk270->update();

	bool hideTank = true;
	if (unkBC) {
		if (isBossPakkunCameraDemo(unkBC))
			hideTank = false;
	} else if (unkC0) {
		if (((TBossEel*)unkC0)->isInBossEelMoguDemo())
			hideTank = false;
	}
	...
	unk140->updatePaneOffset(40, 0,
	                         unk26A + getOffsetForAboveScreen(unk140));
	...
	if (unk140->isInterpolatorAtZero()) {
		int offset = getOffsetForAboveScreen(unk140)
		             - unk128->getPane()->getBounds().getHeight();
		unk140->updatePaneOffset(40, 0, offset);
	}

	int coinOffset = getOffsetForAboveScreen(unk108)
	                 - unkC8->getPane()->getBounds().getHeight();
	unk108->updatePaneOffset(40, 0, coinOffset);
...
void TGCConsole2::endCameraDemo()
{
	...
	if (!unk2F8->isInterpolatorAtZero() && !unk34[17]
		...
		unk2F8->getPane()->show();
		unk2F8->setPaneOffset(unk98, 0, 0, 0,
		                       getOffsetForBelowScreen(unk2F8));
...
void TGCConsole2::startAppearTelop(bool param_1)
{
	...
	if (param_1) {
		// Codegen note: register allocation differs in the message lookup.
...
void TGCConsole2::startInsertJetBalloon()
{
	...
	if (unk404 == unk408)
		...
		unk3FC->setPaneOffset(80, 0, -73, 0,
		                       getOffsetForBelowScreen(unk3FC));
}
...
void TGCConsole2::startAppearRedCoin()
{
	...
	if (unk44C->getPane()->isVisible())
		unk428->setPaneOffset(40, 0, -73, 0,
		                       getOffsetForBelowScreen(unk428));
	else
...
bool TGCConsole2::startAppearBalloon(u32 messageID, bool param_2)
{
	...
	((JSUMemoryOutputStream*)unk3DC)
	    ->setBuffer(unk3B8->getStringPtr(), 0x400);

	const u8* messageText = unk3D0->getMessageData() + entry->unk0;
	((JSUMemoryInputStream*)unk3D4)->setBuffer(messageText, 0x400);

	unk3E0 = messageID;
	size_t length = strlen((const char*)messageText);
	unk3F8 = param_2;
	unk3E4 = (s32)(length * unk3EC + unk3E8);
...
void TGCConsole2::startAppearStar()
{
	...
	unk140->getPane()->show();
	unk128->setPanePosition(50, cDownTopPoint, cDownMidPoint,
	                         cDownMidPoint);
	...
	unk160->getPane()->show();
	unk148->setPanePosition(50, cDownTopPoint, cDownMidPoint,
	                         cDownMidPoint);
	...
	unkC8->setPanePosition(50, JUTPoint(0, 0), cCoinMidPoint,
	                        cCoinMidPoint);
...
bool TGCConsole2::processAppearLife(int param_1)
{
	...
	if (param_1 == 0) {
		unk1C4->setPanePosition(40, JUTPoint(unk1C8, unk1CA),
		                         JUTPoint(-10, 10), JUTPoint(0, 0));
		unk1C4->getPane()->show();
		unk1C4->update();
		isFinished = false;
	} else if (param_1 < 100) {
...
bool TGCConsole2::processDownCoin(int param_1)
{
	...
	for (int i = 0; i < 3; ++i) {
		if (param_1 == i * 6 + 28)
			unkD4[i]->setPanePosition(50, JUTPoint(0, i * 6),
...
bool TGCConsole2::processAppearTank(int param_1)
{
	...
	if (param_1 == 26) {
		unk274->getPane()->show();
		unk274->setPanePosition(40, JUTPoint(0, 100), cUpMidPoint,
		                         cUpMidPoint);
	}
...
bool TGCConsole2::processAppearCoin(int param_1)
{
	...
	if (param_1 == 14) {
		unkCC->getPane()->show();
		unkCC->setPanePosition(50, cDownTopPoint, cDownMidPoint,
		                        cDownMidPoint);
	}

	if (param_1 == 24) {
		unkD0->getPane()->show();
		unkD0->setPanePosition(50, cDownTopPoint, cDownMidPoint,
		                        cDownMidPoint);
	}
...
bool TGCConsole2::processAppearRed(int param_1)
{
	...
	if (unk42C->update()) {
		bool atOrigin = false;
		if (unk42C->unk14.x1 == 0 && unk42C->unk14.y1 == 0)
			atOrigin = true;
		if (!atOrigin) {
			unk42C->setPanePosition(30, cUpMidPoint, cUpMidPoint,
			                         cUpBotPoint);
			isFinished = false;
		}
	} else {
	...
	if (unk430->update()) {
		bool atOrigin = false;
		if (unk430->unk14.x1 == 0 && unk430->unk14.y1 == 0)
			atOrigin = true;
		if (!atOrigin) {
			unk430->setPanePosition(30, cUpMidPoint, cUpMidPoint,
			                         cUpBotPoint);
			isFinished = false;
		}
	} else {
	...
	if (unk434->update()) {
		bool atOrigin = false;
		if (unk434->unk14.x1 == 0 && unk434->unk14.y1 == 0)
			atOrigin = true;
		if (!atOrigin) {
			unk434->setPanePosition(30, cUpMidPoint, cUpMidPoint,
			                         cUpBotPoint);
			isFinished = false;
		}
	} else {
	...
	for (int i = 0; i < 2; ++i) {
		if (unk43C[i]->update()) {
			bool atOrigin = false;
			if (unk43C[i]->unk14.x1 == 0 && unk43C[i]->unk14.y1 == 0)
				atOrigin = true;
			if (!atOrigin) {
				unk43C[i]->setPanePosition(30, cUpMidPoint, cUpMidPoint,
				                            cUpBotPoint);
				isFinished = false;
			}
		} else {
	...
	if (unk438->update()) {
		bool atOrigin = false;
		if (unk438->unk14.x1 == 0 && unk438->unk14.y1 == 0)
			atOrigin = true;
		if (!atOrigin) {
			unk438->setPanePosition(30, cUpMidPoint, cUpMidPoint,
			                         cUpBotPoint);
			isFinished = false;
		}
	} else {
...
bool TGCConsole2::processAppearTimer(int param_1)
{
	...
	if (unk500[0]->isVisible()) {
		...
		unk480[2]->setPanePosition(50, cUpTopPoint, cUpMidPoint,
		                            cUpMidPoint);
	}
...
void TGCConsole2::drawWater(J2DOrthoGraph& graph)
{
	...
	for (int layer = 2; layer > 0; --layer) {
		...
		PSMTXTrans(mtx, 0.0f, unk2B8 * (1.0f - height[layer - 1]), 0.0f);
		GXLoadTexMtxImm(mtx, GX_TEXMTX0, GX_MTX2x4);
		GXSetTexCoordGen2(GX_TEXCOORD0, GX_TG_MTX2x4, GX_TG_TEX0,
		                   GX_TEXMTX0, GX_FALSE, GX_PTIDENTITY);
		GXSetTevOrder(GX_TEVSTAGE0, GX_TEXCOORD0, GX_TEXMAP0,
	...
	if (flags & 1) {
		if (!unk34[28]) {
			if (isConsoleDemoCameraActive() || SMS_CheckMarioFlag(0x400)
			    || (!unk3AC[1]
			        && TFlagManager::smInstance->getBool(0x30002)))
				startCameraDemo();
		} else if (!isConsoleDemoCameraActive()
		...
		if (unk34[16] && unk520->getPane()->isVisible()) {
			...
			if (processDrawTelop(flags)) {
				...
				unk568 = unk544.x2;
				unk534.add(mTelopTextWidth + unk534.x2, 0);
	...
	if (flags & 2) {
		...
		case 1:
			if (processAppearBalloon()) {
				unk10 = 2;
				memset(unk3B8->getStringPtr(), 0, 0x400);
				memset(unk3B4->getStringPtr(), 0, 0x400);
				unk3B8->show();
			}
			break;
		case 2:
		...
		case 4:
			if (processDisappearBalloon()) {
				...
				if (nextMessage != 0xffffffff)
					startAppearBalloon(nextMessage, true);
				unk3F4 = 0xffffffff;
			}
			break;
		}
