...
void TGCConsole2::load(JSUMemoryInputStream& stream)
{
	...
	for (int i = 0; i < 9; ++i) {
		int paneIndex           = i * 2;
		unk17C[paneIndex]       = unkB0->search('lm01' + i * 0x100);
		unk17C[paneIndex + 1]   = unkB0->search('lm02' + i * 0x100);
		unk1D0[i]               = unk17C[paneIndex]->mBounds;
	}
...
bool TGCConsole2::startAppearLife(int param_1)
{
	...
	if (param_1 == 0) {
		setupLifeSegments(this, 0, 9, gpMarioOriginal->mHealth, 0xFFFFFFFF,
		                  0);
		...
		setupLifeSegments(this, 0, 9, (s16)gpMarioOriginal->unk12C,
		                  0x00FFFFFF, 0x003CFF00);
...
bool TGCConsole2::processAppearRed(int param_1)
{
	...
	if (!updateUpPane(unk42C))
		isFinished = false;
	if (!updateUpPane(unk430))
		isFinished = false;
	if (!updateUpPane(unk434))
		isFinished = false;

	for (int i = 0; i < 2; ++i) {
		if (!updateUpPane(unk43C[i]))
			isFinished = false;
	}

	if (!updateUpPane(unk438))
		isFinished = false;
