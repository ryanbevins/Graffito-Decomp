#!/usr/bin/env python3
"""Generate MarNameRefGen_MapObj.cpp from extracted asm entries."""
import io
import re
import sys
...
asm = Path("build/GMSJ01/asm/System/MarNameRefGen_MapObj.s").read_text()
...
# Classes in headers that have a (const char*) ctor and can be used directly
# even when detected via vtable swap.
header_classes_with_name_ctor = {
    "TAirportEventSink", "TAirportPool", "TAirportSwitch", "TBathtub",
    "TBigWatermelon", "TCasinoPanelGate", "TChestRevolve", "TCloset",
    "TCogwheel", "TCogwheelScale", "TCoin", "TCoinBlue", "TCoinEmpty",
    "TCoinRed", "TDemoCannon", "TDonchou", "TDoor", "TDptMonteFence",
    "TEggYoshi", "TFence", "TFerrisWheel", "TFileLoadBlock", "TFluff",
    "TFluffManager", "TGateShadow", "TGoalWatermelon", "THangingBridge",
    "THangingBridgeBoard", "THideObjBase", "THideObjInfo", "THorizontalViking",
    "TItem", "TItemManager", "TItemSlotDrum", "TJointCoin", "TJuiceBlock",
    "TJumpBase", "TLampTrapIron", "TLampTrapSpike", "TLeanBlock", "TLeanMirror",
    "TMammaBlockRotate", "TMammaMirrorMapOperator", "TManhole", "TMapObjBall",
    "TMapObjBase", "TMapObjBaseManager", "TMapObjChangeStage",
    "TMapObjFloatOnSea", "TMapObjGeneral", "TMapObjGrassManager",
    "TMapObjGrowTree", "TMapObjManager", "TMapObjMessenger", "TMapObjNail",
    "TMapObjPlane", "TMapObjPole", "TMapObjPoleManager",
    "TMapObjRevivalPollution", "TMapObjSmoke", "TMapObjSwitch", "TMapObjTree",
    "TMapObjTreeScale", "TMapObjTurn", "TMapObjWaterSpray", "TMapObjWave",
    "TMareGate", "TMerrygoround", "TMonumentShine", "TMuddyBoat", "TNormalLift",
    "TNozzleBox", "TPanelRevolve", "TPictureTelesa", "TPinnaCoaster",
    "TPolluterBase", "TPool", "TPoolManager", "TRailBlock", "TRailFence",
    "TRandomFruit", "TRedCoinSwitch", "TResetFruit", "TRiccoWatermill",
    "TRideCloud", "TRollBlock", "TRoulette", "TSakuCasino",
    "TSandBase", "TSandBird", "TSandBombBase", "TSandCastle",
    "TShellCup", "TShine", "TShiningStone", "TSirenaCasinoRoof",
    "TSirenaRollMapObj", "TSirenabossWall", "TSlotDrum", "TSwingBoard",
    "TTurboNozzleDoor", "TViking", "TWarpAreaActor", "TWaterHitPictureHideObj",
    "TWireBell", "TWoodBarrel", "TWoodBlock",
}
...
if needed_stubs:
    ...
    for _ in range(5):
        ...
        if not changed:
            break
    # Determine ctor signature per stub
    stub_ctor_sig = {}
    for e in entries:
        if not e["ctor"]:
            continue
    ...
    has_weak_dt_set = set(weak_dt_to_parent.keys())
    ...
    for cls in sorted(needed_stubs):
        ...
        has_weak_dt = cls in has_weak_dt_set
...
print("JDrama::TNameRef* TMarNameRefGen::getNameRef_MapObj(const char* name) const", file=out)
print("{", file=out)
for e in entries:
    cmp_s = e["cmp"]
