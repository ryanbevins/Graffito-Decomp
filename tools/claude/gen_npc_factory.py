#!/usr/bin/env python3
"""Generate MarNameRefGen_NPC.cpp from extracted asm entries."""
import io
import re
import sys
...
asm = Path("build/GMSJ01/asm/System/MarNameRefGen_NPC.s").read_text()
...
fn_body = re.search(
    r"\.fn getNameRef_NPC__14TMarNameRefGenCFPCc.*?\n(.*?)\n\.endfn", asm, re.S,
).group(1)
instr = []
...
class_to_header = {
    "TBaseNPC": "NPC/NpcBase.hpp",
    # NpcManager.hpp defines many manager classes with no-arg ctors (incomplete).
    # We need ctors that take (const char*) — exclude these from headers and
    # forward-declare in this TU instead. NpcBase.hpp is fine.
    "TLiveManager": "Strategic/LiveManager.hpp",
...
if needed_stubs:
    ...
    for cls in sorted(needed_stubs):
        ...
        if base != "JDrama::TNameRef" and (base in class_to_header or base in {"JDrama::TNameRef", "THitActor", "TLiveActor"}):
            pad = sz - NAMEREF_BASE_SIZE  # Conservative; ideally sizeof(base)
            ...
            print("public:", file=out)
            print(f"\t{cls}(const char*);", file=out)
            if pad > 0:
                print(f"\tchar _stub[0x{pad:x}];", file=out)
            print("};", file=out)
        print(file=out)

print("JDrama::TNameRef* TMarNameRefGen::getNameRef_NPC(const char* name) const", file=out)
print("{", file=out)
for e in entries:
    cmp_s = e["cmp"]
